...
export default function Title({ label, heading, subtitle }: TitleProps) {
  return (
    <div className="flex flex-col gap-[34px] max-md:gap-5">
      ...
      {heading && (
        <h2 className="font-display font-normal text-[64px] leading-[1em] tracking-[-0.02em] text-dark m-0 max-w-[1088px] max-md:text-4xl">
          {heading}
        </h2>
      )}
      {subtitle && (
        <p className="font-display font-normal text-xl leading-[1.3] text-dark m-0 max-w-[716px] max-md:text-[17px]">
          {subtitle}
        </p>
      )}
    </div>
